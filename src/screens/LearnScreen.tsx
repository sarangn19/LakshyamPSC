...
export function LearnScreen({ navigation }: any) {
  ...
  return (
    <View style={styles.container}>
      ...
      <SelectListModal visible={showChapterModal} slideAnim={chapterSlideAnim} title={t('learn.selectChapter')} searchPlaceholder={t('learn.search')} searchValue={chapterSearch} onSearchChange={setChapterSearch} items={filteredSubjects.map((s) => ({ id: s, title: s }))} selectedId={selectedChapter} onSelect={setSelectedChapter} onContinue={() => { setShowChapterModal(false); animateIn(tasksSlideAnim); setShowTasksModal(true); }} onClose={() => setShowChapterModal(false)} continueDisabled={selectedChapter === null} scrollRef={chapterScrollRef} scrollOffset={chapterScrollOffset} contentHeight={chapterContentHeight} onScroll={(o, h) => { setChapterScrollOffset(o); setChapterContentHeight(h); }} />
